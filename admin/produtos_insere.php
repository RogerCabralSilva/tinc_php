<?php
    ...
    if($_POST){
        if(isset($_POST['enviar'])){
            $nome_img = $_FILES['imagemfile']['name']; // guardando nome do arquivo
            $tmp_img = $_FILES['imagemfile']['tmp_name']; // colocando imagem em um diretorio temporario para verificacao
            $rand = rand(100001,999999);
            $dir_img = "../images/".$rand.$nome_img; // onde vamos guardar a imagem, diretorio
 
            move_uploaded_file($tmp_img,$dir_img); // movendo para o banco de dados
        }
        ...
        $inserindoProduto = "INSERT INTO `produtos` (`tipo_id`, `descricao`, `resumo`, `valor`, `imagem`, `destaque`) VALUES ($id,'$descricao','$resumo',$valor,'$imagem','$destaque')";
   
        $inserindo = $conn->query($inserindoProduto);
    ...
    // selecionar os tipos dos produtos
    $listaTipo = $conn->query('SELECT * FROM tipos ORDER BY rotulo');
    $rowTipo = $listaTipo->fetch_assoc(); // transformando a linha recebida em um array
    $numLinha = $listaTipo->num_rows; // contando as linhas retornadas  
                        ...
                        <form action="produtos_insere.php" method="post" name="form_insere"
                            ...
                            <div class="input-group">
                                ...
                                <select name="id_tipo" id="id_tipo" class="form-control" required>
                                    <?php do {?>
                                    <option value="tipo" <?php echo $rowTipo['id'];?>>
                                        <!-- buscar tipo -->
